...
[~,PM_k] = ginput(1);             %dla ksi_k odczytanego wyżej
...
disp("Zaznacz omegę dla A = "+num2str(A))
...
G_bk = tf(licz_bk,mian_bk);        %układ bez kompensacji

[licz_k,mian_k] = zp2tf(-z,-p,K);    %układ z kompensacją
G_c = tf(licz_k,mian_k);
G_k = series(G_bk,G_c);           

G_k = feedback(G_k,1);             %zamykanie układów
...
xlabel("t")
ylabel("y(t)")
title("Odpowiedź skokowa układu bez kompensacji")

figure(3)
step(G_k)
xlabel("t")
ylabel("y(t)")
title("Odpowiedź skokowa układu z kompensacją")
